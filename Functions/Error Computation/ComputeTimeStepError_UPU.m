function ComputeTimeStepError_UPU()
% ------------------------------------------------------------------------
...
for sim = 1:nsims
    ...
    duf_interp = zeros(Mesh_Exact.nn,1);
    ...
    for i = 1:Mesh_Exact.nn
       ...
       for e = 1:Mesh.ne
           ...
           if coords_ap(1) <= coord_ex && coord_ex <= coords_ap(2)
               ...
               dus_ap = dus(conn_ap);
               % nodal values (fluid displacement)
               duf_ap = duf(conn_ap);
    ...
    for i = 1:Mesh_Exact.ne
        ...
        for ip = 1:Quad.nq
            ...
            % approximated displacement at quadrature point
            ufh = N*duf_interp(Mesh_Exact.xdofs(conn_e));
            % exact displacement at quadrature point
            ufe = N*duf_exact(Mesh_Exact.xdofs(conn_e));
    ...
    eL2uf(sim) = sqrt(eL2uf_num/eL2uf_den);
end
...
nexttile;
loglog(h,eL2uf,'b*', 'LineWidth', 1.5);
hold on
loglog(h,exp(pL2uf(2))*h.^pL2uf(1),'b', 'LineWidth', 1.5);
grid on
xlabel('Mesh size (m)');
ylabel('L2-norm');
title(sprintf('Time - uf Convergence beta = %.2f, order %.4f', beta, m_L2uf));
hold off
