function [u, udot, u2dot, p, pdot, rE] = SolverDynamic(Kuu, Kup, Kpp, M, Mhat, S, Mesh, BC, Control, Iteration)
...
%% Store variable
xE = x(1:length(BC.fixed_u) + length(BC.fixed_p),1);
xF = x(length(BC.fixed_u) + length(BC.fixed_p) + 1: end,1);

u = [xF(1:length(BC.free_u),1); xE(1:length(BC.fixed_u),1)];
p = [xE(length(BC.fixed_u)+1 : end,1); xF(length(BC.free_u)+1 : end,1)];
