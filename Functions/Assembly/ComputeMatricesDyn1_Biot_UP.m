function [Kuu, Kup, Kpp, Muu, Mpu, S] = ComputeMatricesDyn1_Biot_UP(Material, MeshU, MeshP, QuadU, QuadP)
% ------------------------------------------------------------------------
...
for e = 1:ne
    ...
    for ip = 1:nqU
        ...
        Muu_e = Muu_e + Material.rho * (NuVoigt.') * NuVoigt * Material.t * Jdet * QuadU.w(ip,1);
    ...
    for ip = 1:nqP
        ...
        Mpu_e = Mpu_e + Material.rhof * Material.kf * (NpVoigt.') * (Material.m') * BuVoigt * Material.t * Jdet * QuadP.w(ip,1);
    end
