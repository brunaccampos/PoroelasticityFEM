...
function [Mss, Msf, Kss, Ksp, Mfs, Mff, Kfp, Kff, Cps, Kpf, Cpp, Cfs] = ComputeMatricesDyn_dCS_UPW_dimensionless(Material, MeshU, MeshP, QuadU, QuadP)
...
Kpfvec = zeros(ne*MeshP.nDOFe*MeshU.nDOFe,1);
...
for e = 1:ne
    ...
    c1 = 1;
    c2 = Material.M(nMat).rhof*L/sqrt(Material.M(nMat).E * Material.M(nMat).rho);
    c3 = (1-Material.M(nMat).deltaf/Material.M(nMat).eta0)/L/Material.M(nMat).eta0;
    c4 = (1+(Material.M(nMat).deltas-Material.M(nMat).deltaf)/Material.M(nMat).eta0)/L*sqrt(Material.M(nMat).E/Material.M(nMat).rho);
    c5 = Material.M(nMat).rhof/Material.M(nMat).eta0*sqrt(Material.M(nMat).E/Material.M(nMat).rho);
    c6 = Material.M(nMat).rhof*Material.M(nMat).E/Material.M(nMat).rho/L;
    c7 = Material.M(nMat).muf/Material.M(nMat).eta0/L;
    c8 = Material.M(nMat).muf/L^2*sqrt(Material.M(nMat).E/Material.M(nMat).rho);
    c9 = (Material.M(nMat).xif+Material.M(nMat).muf/3-Material.M(nMat).xif*Material.M(nMat).deltaf/Material.M(nMat).eta0)/L/Material.M(nMat).eta0;
    c10 = (Material.M(nMat).eta0*(Material.M(nMat).xif+Material.M(nMat).muf/3) + ...
        Material.M(nMat).xif*Material.M(nMat).deltas - Material.M(nMat).xif*Material.M(nMat).deltaf)...
        /L^2/Material.M(nMat).eta0*sqrt(Material.M(nMat).E/Material.M(nMat).rho);
    ...
    for ip = 1:nqU
        ...
        Kff_e = Kff_e + (Material.M(nMat).muf*L/Material.M(nMat).k * (NuVoigt.') * NuVoigt + ...
            (c7 + c9) * (BuVoigt.') * BuVoigt) * Material.t * Jdet * QuadP.w(ip,1);
        
        Cfs_e = Cfs_e + (c8 + c10) * (BuVoigt.') * BuVoigt * Material.t * Jdet * QuadU.w(ip,1);
        Mss_e = Mss_e + c1 * (NuVoigt.') * NuVoigt * Material.t * Jdet * QuadU.w(ip,1);
        Mff_e = Mff_e + c5 * (NuVoigt.') * NuVoigt * Material.t * Jdet * QuadU.w(ip,1);
        Msf_e = Msf_e + c2 * (NuVoigt.') * NuVoigt * Material.t * Jdet * QuadU.w(ip,1);
        Mfs_e = Mfs_e + c6 * (NuVoigt.') * NuVoigt * Material.t * Jdet * QuadU.w(ip,1);
    end
    ...
    for ip = 1:nqP
        ...
        Kfp_e = Kfp_e + (Material.M(nMat).Kf/L) * (BuVoigt.') * Material.m * NpVoigt * Material.t * Jdet * QuadP.w(ip,1);
        Kpf_e = Kpf_e + c3 * (NpVoigt.') * (Material.m') * BuVoigt * Material.t * Jdet * QuadP.w(ip,1);
        
        Cps_e = Cps_e + c4 * (NpVoigt.') * (Material.m') * BuVoigt * Material.t * Jdet * QuadU.w(ip,1);
        Cpp_e = Cpp_e + (sqrt(Material.M(nMat).E/Material.M(nMat).rho)/L) * (NpVoigt.') * NpVoigt * Material.t * Jdet * QuadU.w(ip,1);
    end
