function [Kuu, Kup, Kpp, M, Mhat, S] = ComputeSystemMatrices_Dynamic(Material, Mesh, Control, Quad)
...
Mvec = zeros(Mesh.ne*Mesh.ndof_u_e^2,1);
...
for e = 1:Mesh.ne
    ...
    M_e = zeros(Mesh.ndof_u_e, Mesh.ndof_u_e);
    ...
    for ip = 1:Control.nq
        ...
        M_e = M_e + Material.rho * (Nu.') * Nu * J * Quad.w_csi(ip,1);
    ...
    M_e = reshape(M_e, [Mesh.ndof_u_e^2,1]);
    ...
    Mvec(count_u-Mesh.ndof_u_e^2:count_u-1) = M_e;
...
M = sparse(rowu, colu, Mvec, Mesh.ndof_u, Mesh.ndof_u);
