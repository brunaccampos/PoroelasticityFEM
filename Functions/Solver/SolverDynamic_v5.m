function [Solution] = SolverDynamic_v5(Muu, Mpu, Mnu, Kuu, Kup, Kpp, Kpu, S, Kpn, Knn, Knu, Knp, Aun, Bun, fu, fp, fn, BC, Control, Iteration)
% ------------------------------------------------------------------------
...
% ------------------------------------------------------------------------
% version 5: adapt for Spanos formulation
% ------------------------------------------------------------------------
...
AunFF = Aun(BC.free_u, BC.free_n);
BunFF = Bun(BC.free_u, BC.free_n);
...
if Control.step == 1
    u2dot_old(BC.free_u) = MuuFF\(-KuuFF*u_old(BC.free_u) + KupFF*p_old(BC.free_p) - AunFF*n_old(BC.free_n) + BunFF*ndot_old(BC.free_n) + fu(BC.free_u));
...
fubar = fu + Muu * (u_old./(beta*dt^2) + udot_old./(beta*dt) + (1/(2*beta)-1) * u2dot_old) - ...
    Bun * (n_old./(lambda*dt) + (1/lambda-1) * ndot_old);
...
fnbar = - Mnu * (u_old./(beta*dt^2) + udot_old ./ (beta*dt) + (1/(2*beta)-1) * u2dot_old) + ...
