...
% ------------------------------------------------------------------------
% version 4 (11/29/22): different quadratures for u-p fields
% ------------------------------------------------------------------------
...
if Control.steady %% quasi-steady/transient case
    if Control.Biotmodel % Biot
        [Kuu, Kup, Kpp, S] = ComputeSystemMatrices_Transient_v4(Material, MeshU, MeshP, QuadU, QuadP);
    else % Spanos
        [Kuu, Kup, Kpp, Kpu, S, Kpn, Knn, Knu, Knp, Kun] = ComputeSystemMatrices_Transient_v5(Material, MeshU, MeshP, MeshN, QuadU, QuadP);
    end
else %% dynamic case
    if Control.Biotmodel % Biot
        [Kuu, Kup, Kpp, M, Mhat, S] = ComputeSystemMatrices_Dynamic(Material, MeshU, MeshP, QuadU, QuadP);
    else % Spanos
        [Muu, Mpu, Mnu, Kuu, Kup, Kpp, Kpu, S, Kpn, Knn, Knu, Knp, Kun] = ComputeSystemMatrices_Dynamic_v3(Material, MeshU, MeshP, MeshN, QuadU);
    end
end

%% Assemble system load vectors
[fu,fp,fn] = ComputeSystemLoads_v2(BC, MeshU, MeshP, MeshN, Control, QuadU, QuadP);
...
%             u_an = @(x) (x.^5 - x.^4);
%             u_an = @(x) (-x.^2 + 2*max(MeshU.coords).*x);
%             u_an = @(x) -x.^3 + x.^2 +(3*max(MeshU.coords)^2 - 2*max(MeshU.coords)).*x;
%             u_an = @(x) -x.^2 + 2*max(MeshU.coords).*x;
