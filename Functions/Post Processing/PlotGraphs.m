function PlotGraphs(MeshU, MeshP, MeshN, Control, Plot, u, p, saveGraphs_on)
...
if Control.steady
    ...
    if ~Control.Biotmodel
        figure;
        plot(Plot.time, Plot.n,'g','LineWidth',2);
        hold on
        xlabel('Time [s]');
        ylabel('Porosity [-]');
        title(sprintf('Porosity at x = %.2f m', MeshN.coords(Control.plotp,1)));
        hold off
        if saveGraphs_on
            exportgraphics(gcf,'Press_time.png','Resolution',300)
        end
