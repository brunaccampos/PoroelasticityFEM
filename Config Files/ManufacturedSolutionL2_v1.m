function [Material, MeshU, MeshP, MeshN, BC, Control] = ManufacturedSolutionL2_v1(config_dir, progress_on)
% ------------------------------------------------------------------------
% Manufactured solution for L2 element mesh size convergence study
% ux = -x^3 + x^2 +(3L^2-2L)*x
% ------------------------------------------------------------------------
...
%% Dirichlet BCs - solid
BC.ux = @(x) -x.^3 + x.^2 +(3*max(MeshU.coords)^2 - 2*max(MeshU.coords)).*x;
