function [Material, MeshU, MeshP, MeshN, BC, Control] = PlatePointInjection_Dynamic(config_dir, progress_on,~,~)
...
% point flux [m/s]
nodePmiddle = find(MeshP.coords(:,1) == 7.5 & MeshP.coords(:,2) == 7.5);
BC.pointFluxValue = -0.01;
BC.pointFluxNodes = nodePmiddle;
...
% k = 1; % function parameter 
% 
% for n = 1:MeshP.nn % loop over all nodes
%     coords = MeshP.coords(n,:); % point coord
%     dist = sqrt((coords(1,1) - centre(1,1))^2 + (coords(1,2) - centre(1,2))^2); % distance between node and centre
%     if dist < r % check if node is within radius distance
%         w = exp(-k*dist^2/r^2); % compute weight
%         BC.pointFlux(n) = BC.pointFluxValue * w; % compute weighted flux
%     end
% end
...
%% Plot graphs
% DOF to plot graphs
nodeUmiddle = find(MeshU.coords(:,1) == 7.5 & MeshU.coords(:,2) == 7.5);
Control.plotu = nodeUmiddle*2; % dof y at middle node
