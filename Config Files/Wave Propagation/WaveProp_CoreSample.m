function [Material, MeshU, MeshP, MeshN, BC, Control] = WaveProp_CoreSample(config_dir, progress_on,~,~)
...
%% Dirichlet BCs - solid
% central node
node = find(MeshU.coords(:,1) == 0 & MeshU.coords(:,2) == 0.05);
% node y DOF
...
BC.fixed_w = [MeshU.left_dofx; MeshU.right_dofx];
BC.fixed_w_value = @(t) zeros(length(BC.fixed_w),1);
% free displacement nodes
BC.free_w = setdiff(MeshU.DOF, BC.fixed_w);

%% Dirichlet BCs - fluid
BC.fixed_p = [MeshP.top_nodes; MeshP.bottom_nodes];
...
nodeP = find(MeshP.coords(:,1) == 0 & MeshP.coords(:,2) == 0.05);
