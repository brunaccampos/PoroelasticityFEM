...
function [Material, MeshU, MeshP, MeshN, BC, Control] = WaveProp_TestFFT(config_dir, progress_on,~,~)
...
%%%% displacement field
fieldU = 'u';
meshFileNameU = 'Mesh Files\LayeredDomain_2000x4000.msh';
MeshU = BuildMesh_GMSH(meshFileNameU, fieldU, nsd, config_dir, progress_on);

%%%% pressure field
fieldP = 'p';
meshFileNameP = 'Mesh Files\LayeredDomain_2000x4000.msh';
...
f = 10;
% fixed DOF values
BC.fixed_u_value = @(t) [(sin(pi*(t)*f) - 0.5*sin(2*pi*(t)*f)).*(t<1/f); zeros(length(BC.fixed_u)-1,1)];
...
Control.tend = 0.32;   % final simulation time
...
Control.plotSyntheticScale = 5;

% Plot in a row
Control.fixedDepthPlotON = 1; % 0: false, 1: true

Control.depthplot = 1100; % fixed coordinate
