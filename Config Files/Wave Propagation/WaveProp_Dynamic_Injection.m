function [Material, MeshU, MeshP, MeshN, BC, Control] = WaveProp_Dynamic_Injection(config_dir, progress_on)
...
switch MeshType
    ...
    case 'Gmsh'
        ...
        %%%% displacement field
        fieldU = 'u';
        meshFileNameU = 'Mesh Files\WavePropInj1x1mQ4.msh';
        MeshU = BuildMesh_GMSH(meshFileNameU, fieldU, nsd, config_dir, progress_on);
        %%%% pressure field
        fieldP = 'p';
        meshFileNameP = 'Mesh Files\WavePropInj1x1mQ4.msh';
        MeshP = BuildMesh_GMSH(meshFileNameP, fieldP, nsd, config_dir, progress_on);
        %%%% porosity field
        if contains(Control.PMmodel, 'UPN')
            fieldN = 'n';
            meshFileNameN = 'Mesh Files\WavePropInj1x1mQ4.msh';
...
MeshP.nodesWell = 5:45; 
% fixed DOFs 
BC.fixed_p = MeshP.nodesWell;
% fixed DOF values
t0=1e-3; % period [s]
BC.fixed_p_value = @(t)(sin(2*pi*(t)/t0) - 0.5*sin(4*pi*(t)/t0)).*(t<t0);
% free nodes
...
%% Plot data
% DOF to plot graphs
Control.plotu = 26*2; % point at x=7.46, y=7.45
Control.plotp = 26; % point at x=7.46, y=7.45
