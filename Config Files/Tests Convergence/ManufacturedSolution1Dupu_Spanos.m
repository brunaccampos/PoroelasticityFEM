...
% uf
BC.initUf = cos(F1.*MeshU.coords)./1000;
% p
BC.initP = sin(F1.*MeshP.coords)./1000;
% usdot
BC.initUdot = F2*cos(F1.*MeshU.coords)./1000;
% ufdot
BC.initUfdot = -F2*sin(F1.*MeshU.coords)./1000;
...
BC.fixed_uf_value = @(t) [cos(F2*t)*ones(length(MeshU.left_nodes),1); cos(F1*L+F2*t)*ones(length(MeshU.right_nodes),1)]./1000;
% free displacement nodes
...
% body force [GN/m3]
BC.bs = @(x,t) (Material.M(1).E*F1^2*sin(F1*x+F2*t) + (Material.M(1).alpha-Material.M(1).eta0)*F1*cos(F1*x)- ...
    (1-Material.M(1).eta0)*Material.M(1).rhos*F2^2*sin(F1*x+F2*t) + ...
    (Material.M(1).muf*Material.M(1).eta0^2/Material.M(1).k)*F2*(sin(F1*x+F2*t)+cos(F1*x+F2*t)) + ...
    (4*Material.M(1).eta0*Material.M(1).muf/3+Material.M(1).eta0*Material.M(1).xif-Material.M(1).xif*Material.M(1).deltaf)*F1^2*F2*sin(F1*x+F2*t) - ...
    Material.M(1).xif*Material.M(1).deltas*F1^2*F2*cos(F1*x+F2*t))./1000;

BC.bf = @(x,t) (Material.M(1).eta0*F1*cos(F1*x) - Material.M(1).eta0*Material.M(1).rhof*F2^2*cos(F1*x+F2*t) -...
    (Material.M(1).muf*Material.M(1).eta0^2/Material.M(1).k)*F2*(sin(F1*x+F2*t)+cos(F1*x+F2*t)) - ...
    (4*Material.M(1).eta0*Material.M(1).muf/3+Material.M(1).eta0*Material.M(1).xif-Material.M(1).xif*Material.M(1).deltaf)*F1^2*F2*sin(F1*x+F2*t) + ...
    Material.M(1).xif*Material.M(1).deltas*F1^2*F2*cos(F1*x+F2*t))./1000;
...
% flux source [m3/s/m3]
BC.s = @(x,t) (Material.M(1).deltas*F1/Material.M(1).eta0*cos(F1*x+F2*t) - ...
    (1-Material.M(1).deltaf/Material.M(1).eta0)*F1*sin(F1*x+F2*t) +...
    (1/Material.M(1).Kf)*(sin(F1*x)+sin(F2*t)))./1000;
...
Control.ufan_symb = @(x,t) cos(F1*x + F2*t)./1000;
