function [Material, MeshU, MeshP, MeshN, BC, Control] = ManufacturedSolution1Dup_Biot_v2(~, progress_on,~,~)
% ------------------------------------------------------------------------
...
% body force [GN/m3]
BC.b = @(x,t) (Material.M(1).E*F1^2*sin(F1*x+F2*t) + Material.M(1).alpha*F1*cos(F1*x))./1000;
...
% flux source [m3/s/m3]
BC.s = @(x,t) (Material.M(1).k/Material.M(1).muf*F1^2*sin(F1*x) - Material.M(1).alpha*F1*F2*sin(F1*x+F2*t) + ...
    Material.M(1).Minv*F2*cos(F2*t))./1000;
