...
%% Initial conditions
% usdot
BC.initUdot = pi*cos(pi.*MeshU.coords)./1000;
% ufdot
BC.initUfdot = pi*sin(pi.*MeshU.coords)./1000;

%% Dirichlet BCs - solid
% displacement prescribed on the left and right
BC.fixed_u = [MeshU.left_nodes; MeshU.right_nodes];
BC.fixed_u_value = @(t) [sin(pi*t).*ones(length(MeshU.left_nodes),1); cos(pi*L).*sin(pi*t).*ones(length(MeshU.right_nodes),1)]./1000;
...
BC.fixed_uf_value = @(t) [zeros(length(MeshU.left_nodes),1); sin(pi*L).*sin(pi*t).*ones(length(MeshU.right_nodes),1)]./1000;
% free displacement nodes
...
BC.fixed_p_value = @(t) [zeros(length(MeshP.left_nodes),1); cos(pi*L).*sin(pi*L).*sin(pi*t).*ones(length(MeshP.right_nodes),1)]./1000;
% free pressure nodes
...
% flux source [m3/s/m3]
BC.s = @(x,t) (cos(pi*x)*sin(pi*x)*sin(pi*t)./Material.Kf - Material.deltas*pi/Material.eta0*sin(pi*x)*sin(pi*t) + ...
    (1-Material.deltaf/Material.eta0)*pi*cos(pi*x)*sin(pi*t))./1000;
...
% solution in u
Control.uan_symb = @(x,t) cos(pi*x).*sin(pi*t)./1000;
Control.u_an = @(t) Control.uan_symb(MeshU.coords,t);

Control.ufan_symb = @(x,t) sin(pi*x).*sin(pi*t)./1000;
Control.uf_an = @(t) Control.ufan_symb(MeshU.coords,t);

% solution in p
Control.pan_symb = @(x,t) cos(pi*x).*sin(pi*x).*sin(pi*t)./1000;
Control.p_an = @(t) Control.pan_symb(MeshP.coords,t);

%% Time step controls
Control.dt = 1e-3;  % time step [s]
Control.tend = 1e-1;   % final simulation time [s]
