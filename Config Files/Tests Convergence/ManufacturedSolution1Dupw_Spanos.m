...
% Dyn8_Spanos_UPW ---- Spanos model (u-p-w), dynamic, u = solid displacement,
%                      p = fluid pressure, w = relative fluid velocity
Control.PMmodel = 'Dyn8_Spanos_UPW';
...
% additional mass coefficient
Material.M(1).rho12 = 0;
...
% ufdot
BC.initW = ones(MeshU.nDOF,1)./1000;
...
% displacement prescribed on the left and right
BC.fixed_w = [MeshU.left_nodes; MeshU.right_nodes];
BC.fixed_w_value = @(t) [1; cos(L*t)]./1000;
% free displacement nodes
...
% body force [GN/m3]
BC.bs = @(x,t) (Material.M(1).E * t^2 * sin(x*t) + Material.M(1).alpha*t*(cos(x*t)^2-sin(x*t)^2) -...
    Material.M(1).rho*x^2*sin(x*t) - Material.M(1).rhof*x*sin(x*t))./1000;

BC.bf = @(x,t) (t*(cos(x*t)^2-sin(x*t)^2) - Material.M(1).rhof*x^2*sin(x*t) + ...
    Material.M(1).muf/Material.M(1).k*cos(x*t) - Material.M(1).rhof/Material.M(1).eta0*x*sin(x*t) + ...
    1/Material.M(1).eta0*(4*Material.M(1).muf/3+Material.M(1).xif-...
    Material.M(1).xif*Material.M(1).deltaf/Material.M(1).eta0)*t^2*cos(x*t) + ...
    1/Material.M(1).eta0*(Material.M(1).muf*Material.M(1).eta0+Material.M(1).eta0*...
    (Material.M(1).xif+Material.M(1).muf/3)+Material.M(1).xif*Material.M(1).deltas-...
    Material.M(1).xif*Material.M(1).deltaf) *(2*t*sin(x*t) + x*t^2*cos(x*t)))./1000;
...
% flux source [m3/s/m3]
BC.s = @(x,t) (x./Material.M(1).Kf *(cos(x*t)^2-sin(x*t)^2) - ...
    (1-Material.M(1).deltaf/Material.M(1).eta0)*t./Material.M(1).eta0 * sin(x*t) + ...
    (1+(Material.M(1).deltas-Material.M(1).deltaf)/Material.M(1).eta0) * (cos(x*t)-x*t*sin(x*t)))./1000;
...
% materials (1/M=0)
Control.ansol_type = 'getAnSol_uncoupled_UPW';

% solution in u
Control.uan_symb = @(x,t) sin(x*t)./1000;
Control.u_an = @(t) Control.uan_symb(MeshU.coords,t);

Control.wan_symb = @(x,t) cos(x*t)./1000;
Control.w_an = @(t) Control.wan_symb(MeshU.coords,t);
