function [Material, MeshU, MeshP, MeshN, BC, Control] = ManufacturedSolution1Dupu_Biot(~, progress_on,~,~)
% ------------------------------------------------------------------------
...
BC.bs = @(x,t) (Material.M(1).E*F1^2*sin(F1*x+F2*t) + (Material.M(1).alpha-Material.M(1).eta0)*F1*cos(F1*x)- ...
    ...
    (Material.M(1).muf*Material.M(1).eta0^2/Material.M(1).k)*F2*(sin(F1*x+F2*t)+cos(F1*x+F2*t)))./1000;
...
% flux source [m3/s/m3]
BC.s = @(x,t) (-Material.M(1).eta0*F1*sin(F1*x+F2*t) + (Material.M(1).alpha-Material.M(1).eta0)*F1*cos(F1*x+F2*t) +...
    Material.M(1).Minv*(sin(F1*x)+sin(F2*t)))./1000;
...
% materials (1/M=0)
Control.ansol_type = 'getAnSol_uncoupled_UPU';
